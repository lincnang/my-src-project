...
public class ArmorEnchantSystem {
    ...
    // 取得強化防具指定編號 2024.4.1 拿掉
    //	public ArmorEnchantSystem get(final int armor , final int Evolevel) {
    //		try {
    //			if (Evolevel == 0) {
    //				return null;
    //			}
    ...
    //					final ArmorEnchantSystem list = _enchantlist.get(i);
    //					if (list != null) {
    //						if (list.getArmorId() == armor &&
    ...
    public static ArmorEnchantSystem get() {
        if (_instance == null) {
            _instance = new ArmorEnchantSystem();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            pm = co.prepareStatement("SELECT * FROM `系統_強化防具加乘`");
            rs = pm.executeQuery();
            while (rs.next()) {
                final int id = (rs.getInt("序號"));
                ArmorEnchantSystem ae = new ArmorEnchantSystem();
                ae.setArmorId(rs.getInt("防具編號"));
                ae.setEnchant(rs.getInt("level"));
        ...
        _log.info("讀取->系統_強化防具加乘數量: " + _enchantlist.size() + "(" + timer.get() + "ms)");
    }

    public ArmorEnchantSystem get2(final int safeEnchantLevel, final int Evolevel) {
        try {
            if (Evolevel == 0) {
                return null;
            }
            if (_enchantlist.size() > 0) {
                for (int i = _enchantlist.size(); i >= 0; i--) {
                    final ArmorEnchantSystem list = _enchantlist.get(i);
                    if (list != null) {
                        if (list.getArmorId() == 0 && (Evolevel - safeEnchantLevel) >= list.getEnchant()) {
                            return list;
                        }
                    }
                }
