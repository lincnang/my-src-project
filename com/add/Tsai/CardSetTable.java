...
public class CardSetTable {
    ...
    private static final HashMap<Integer, CardPolySet> _cardIndex = new HashMap<>();
    ...
    public void load() {
        ...
        try {
            ...
            ps = cn.prepareStatement("SELECT * FROM `系統_變身組合套卡`");
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                final CardPolySet card = new CardPolySet(id, msg1, needids, needquests, neednames, questid, str, dex, con, Int, wis, cha, ac, hp, mp, hpr, mpr, dmg, bowdmg, hit, bowhit, dmgr, mdmgr, sp, mhit, mr, fire, water, wind, earth);
        ...
        _log.info("讀取->系統_變身組合套卡: " + t + "(" + timer.get() + "ms)");
    }
    ...
    public CardPolySet getCard(final int id) {
