 ...
 * @author hero
 */
public class dollSetTable {
    private static final Log _log = LogFactory.getLog(dollSetTable.class);
    private static final HashMap<Integer, dollPolySet> _dollIndex = new HashMap<>();
    private static dollSetTable _instance;

    public static dollSetTable get() {
        if (_instance == null) {
            _instance = new dollSetTable();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            ps = cn.prepareStatement("SELECT * FROM `系統_卡片娃娃套卡`");
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                final dollPolySet doll = new dollPolySet(id, msg1, needids, needquests, neednames, questid, str, dex, con, Int, wis, cha, ac, hp, mp, hpr, mpr, dmg, bowdmg, hit, bowhit, dmgr, mdmgr, sp, mhit, mr, fire, water, wind, earth);
        ...
        _log.info("讀取->系統_卡片娃娃套卡: " + t + "(" + timer.get() + "ms)");
    }

    public int CardDollSize() {
        return _dollIndex.size();
    }

    public dollPolySet getDoll(final int id) {
        return _dollIndex.get(id);
    }

    public Integer[] getDollIds() {
