 ...
 * @author hero
 */
public class dollTable {
    private static final Log _log = LogFactory.getLog(dollTable.class);
    private static final HashMap<Integer, doll> _dollIndex = new HashMap<>();
    private static dollTable _instance;

    public static dollTable get() {
        if (_instance == null) {
            _instance = new dollTable();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            ps = cn.prepareStatement("SELECT * FROM `系統_卡片娃娃`");
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                final int dollidid = rs.getInt("對應的娃娃編號");
                final int dollitemid = rs.getInt("娃娃消耗道具編號");
                final int dolltemcount = rs.getInt("娃娃消耗道具數量");
                ...
                final doll doll = new doll(id, group, msg1, msg2, cmd, questid, dollidid, dollitemid, dolltemcount, str, dex, con, Int, wis, cha, ac, hp, mp, hpr, mpr, dmg, bowdmg, hit, bowhit, dmgr, mdmgr, sp, mhit, mr, fire, water, wind, earth, hgfx, cgfx, shanbi, huibi, yaoshui, fuzhong, exp, hunmi, zhicheng, shihua, hanbing, anhei, shuimian);
                _dollIndex.put(id, doll);
                i++;
            }
        ...
        _log.info("讀取->系統_卡片娃娃: " + i + "(" + timer.get() + "ms)");
    }

    public int dollSize() {
        return _dollIndex.size();
    }

    // 取得已獲得娃娃總數
    public int getDollHaveGroup(L1PcInstance pc, int group) {
        int count = 0;
        for (doll doll : _dollIndex.values()) {
            if (doll.getGroup() == group && dollQuestTable.get().IsQuest(pc, doll.getQuestId())) {
                count++;
            }
        }
        return count;
    ...
    // 取得群組總數
    public int getDollGroupSize(int group) {
        int count = 0;
        for (doll doll : _dollIndex.values()) {
            if (doll.getGroup() == group) {
                count++;
            }
        }
        return count;
    ...
    public doll getDoll(final int id) {
        return _dollIndex.get(id);
    }

    public int getQuestIdByDollId(int dollId) {
        for (doll doll : _dollIndex.values()) {
            if (doll.getId() == dollId) {
                return doll.getQuestId();
            }
        }
        return 0;
    }

    public Integer[] getDollsIndex() {
        return _dollIndex.keySet().toArray(new Integer[0]);
    }
}