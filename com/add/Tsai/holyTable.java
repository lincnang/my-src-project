 ...
 * @author hero
 */
public class holyTable {
    private static final Log _log = LogFactory.getLog(holyTable.class);
    private static final HashMap<Integer, holy> _holyIndex = new HashMap<>();
    private static holyTable _instance;

    public static holyTable get() {
        if (_instance == null) {
            _instance = new holyTable();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            ps = cn.prepareStatement("SELECT * FROM `系統_聖物`");
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                final int holyidid = rs.getInt("對應的娃娃編號");
                final int holyitemid = rs.getInt("娃娃消耗道具編號");
                final int holytemcount = rs.getInt("娃娃消耗道具數量");
                ...
                final holy holy = new holy(id, group, msg1, msg2, cmd, questid, holyidid, holyitemid, holytemcount, str, dex, con, Int, wis, cha, ac, hp, mp, hpr, mpr, dmg, bowdmg, hit, bowhit, dmgr, mdmgr, sp, mhit, mr, fire, water, wind, earth, hgfx, cgfx, shanbi, huibi, yaoshui, fuzhong, exp, hunmi, zhicheng, shihua, hanbing, anhei, shuimian);
        ...
        _log.info("讀取->系統_聖物: " + i + "(" + timer.get() + "ms)");
    }
    ...
    // 取得已獲得聖物總數
    public int getHolyHaveGroup(L1PcInstance pc, int group) {
        int count = 0;
        for (holy holy : _holyIndex.values()) {
            if (holy.getGroup() == group && dollQuestTable.get().IsQuest(pc, holy.getQuestId())) {
                count++;
            }
        }
        return count;
    ...
    // 取得群組總數
    public int getHolyGroupSize(int group) {
        int count = 0;
        for (holy holy : _holyIndex.values()) {
            if (holy.getGroup() == group) {
                count++;
            }
        }
        return count;
    ...
    public holy getHoly(final int id) {
        return _holyIndex.get(id);
    }

    public int getQuestIdByHolyId(int holyId) {
        for (holy holy : _holyIndex.values()) {
            if (holy.getId() == holyId) {
                return holy.getQuestId();
            }
        }
        return 0;
    }

    public Integer[] getHolyIndex() {
