...
public final class Configpoly {
    ...
    public static int[] poly_LIST_6;//給與變身卡列陣
    ...
    public static int ZUIDAJILV6;//合成六階段最大幾率
    public static int ZUIXIAOJILV6;//合成六階段最小几率
    public static int ZENGJIAJILV6;//合成六階段增加幾率數量
    public static int SHULIANG6;//合成六階段需要的變身卡數量
    ...
    public static List<Integer> poly5 = new ArrayList<>();//倉庫封包判定5階段
    ...
    public static void load() throws ConfigErrorException {
        ...
        try {
            ...
            if (set.getProperty("poly5") != null) {
                for (final String str : set.getProperty("poly5").split(",")) {
                    poly5.add(Integer.parseInt(str));
                }
            }
        } catch (Exception e) {
            _log.error("變身卡合成設定加載失敗: " + e.getMessage(), e);
    ...
    public static int[] toIntArray(final String text, final String type) {
        ...
        int[] iReturn = new int[list.size()];
        for (int i = 0; i < list.size(); i++) {
            iReturn[i] = list.get(i);
        }
        return iReturn;
    }
}
