...
public class BossWeapon extends ItemExecutor {
    ...
    public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
        ...
        L1BossWeapon bossWeapon = ExtraBossWeaponTable.getInstance().get(item.getItemId(), 1);
        if (bossWeapon == null) {
        ...
        if (power == null) {
            ...
            if ((_random.nextInt(1000) >= bossWeapon.getSuccessRandom())) {
                pc.sendPackets(new S_SystemMessage(bossWeapon.getFailureMsg()));
                return;
            }
            ...
            final L1BossWeapon srcBossWeaopn = ExtraBossWeaponTable.getInstance().get(item.getItemId(), power.get_boss_lv());
            if (srcBossWeaopn == null) {
                _log.error("------ 當前頭目武器升級錯誤 ------");
                return;
            }
            if (power.get_boss_weapon() == srcBossWeaopn) {
                final L1BossWeapon nextBossWeaopn = ExtraBossWeaponTable.getInstance().get(item.getItemId(), power.get_boss_lv() + 1);
                if (nextBossWeaopn == null) {
                    pc.sendPackets(new S_SystemMessage("頭目靈魂等級已經是最大值。"));
                    return;
                }
                pc.getInventory().removeItem(item, 1);
                if (_random.nextInt(1000) >= nextBossWeaopn.getSuccessRandom()) {
                    final L1BossWeapon beginBossWeaopn = ExtraBossWeaponTable.getInstance().get(item.getItemId(), 1);
                    final L1BossWeapon backBossWeaopn = ExtraBossWeaponTable.getInstance().get(item.getItemId(), power.get_boss_lv() - 1);
                    pc.sendPackets(new S_SystemMessage(nextBossWeaopn.getFailureMsg()));
                ...
                if (ConfigWeaponArmor.Cross_BOSS) {
                    final L1BossWeapon newBossWeapon = ExtraBossWeaponTable.getInstance().get(item.getItemId(), 1);
                    if (newBossWeapon == null) {
                        _log.error("------- 新的一筆頭目靈魂武器錯誤 --------");
                        return;
                    }
                    pc.getInventory().removeItem(item, 1);
                    if (_random.nextInt(1000) >= newBossWeapon.getSuccessRandom()) {
                        pc.sendPackets(new S_SystemMessage(newBossWeapon.getFailureMsg()));
                        return;
                    }
                    power.set_boss_lv(1);
