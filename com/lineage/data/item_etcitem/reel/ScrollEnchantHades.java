...
public class ScrollEnchantHades extends ItemExecutor {
    ...
    public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
        ...
        if (isEnchant) {// 成功
            ...
            WriteLogTxt.Recording("強化防具紀錄", "IP(" + pc.getNetConnection().getIp() + ")玩家【" + pc.getName() + "】的【" + tgItem.getRecordName(tgItem.getCount()) + ", (ObjId: " + item.getId() + ")】強化成功.");
        } else {// 失敗
            enchantExecutor.failureEnchant(pc, tgItem);
            WriteLogTxt.Recording("強化防具紀錄", "IP(" + pc.getNetConnection().getIp() + ")玩家【" + pc.getName() + "】的【" + tgItem.getRecordName(tgItem.getCount()) + ", (ObjId: " + item.getId() + ")】強化失敗.");
        }
    }
}