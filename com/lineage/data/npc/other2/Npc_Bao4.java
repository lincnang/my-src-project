        ...
        try {
            if (cmd.equalsIgnoreCase("bao_quest4")) {
                ...
                if (pc.getMeteLevel() < ConfigQuest.Bao4_Quest_Levelmet) {
                    pc.sendPackets(new S_SystemMessage("您未達" + ConfigQuest.Bao4_Quest_Levelmet + "轉，無法接取任務。"));
                    return;
                }
                if (pc.getLevel() + ((pc.getMeteLevel() - ConfigQuest.Bao4_Quest_Levelmet) * 99) < ConfigQuest.Bao4_Quest_Lv) {
                    pc.sendPackets(new S_SystemMessage("您未達" + ConfigQuest.Bao4_Quest_Lv + "等，無法接取任務。"));
                    return;
                }
                pc.getQuest().set_step(L1PcQuest.BAO_QUEST_4, L1PcQuest.QUEST_START);
