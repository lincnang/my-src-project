...
public final class InvSwapTable {
    ...
    private InvSwapTable() {
        ...
        try {
            ...
            st = con.prepareStatement("SELECT * FROM 日誌_裝備切換");
            rs = st.executeQuery();
            while (rs.next()) {
        ...
        synchronized (list) {
            for (int key : list.keySet()) {
                try {
                    //
                    con = DatabaseFactory.get().getConnection();
                    st = con.prepareStatement("DELETE FROM 日誌_裝備切換 WHERE objectId=?");
                    st.setInt(1, key);
                    st.executeUpdate();
                    st.close();
                    //
                    ...
                    //
                    st = con.prepareStatement("INSERT INTO 日誌_裝備切換 SET objectId=?, setCode=?, set1=?, set2=?");
                    st.setInt(1, key);
                    st.setInt(2, code);
