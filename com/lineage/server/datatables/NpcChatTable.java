...
public class NpcChatTable {
    ...
    public void load() {
        ...
        _log.info("讀取->NPC會話資料數量: " + (_npcChatAppearance.size() + _npcChatDead.size() + _npcChatHide.size() + _npcChatGameTime.size()) + "(" + timer.get() + "ms)");
    }
    ...
    public L1NpcChat[] getAllGameTime() {
        return _npcChatGameTime.values().toArray(new L1NpcChat[0]);
    }
}
