...
public class DwarfForElfTable implements DwarfForElfStorage {
    ...
    public void load() {
        ...
        _log.info("讀取->精靈倉庫物件清單資料數量: " + _itemList.size() + "/" + i + "(" + timer.get() + "ms)");
    }
    ...
    public void insertItem(String account_name, L1ItemInstance item) {
        _log.warn("帳號:" + account_name + " 加入妖精倉庫數據:" + item.getNumberedName(item.getCount(), false) + " OBJID:" + item.getId());
        addItem(account_name, item);
        Connection con = null;
        PreparedStatement pstm = null;
        try {
    ...
    public void deleteItem(String account_name, L1ItemInstance item) {
        CopyOnWriteArrayList<L1ItemInstance> list = _itemList.get(account_name);
        if (list != null) {
            _log.warn("帳號:" + account_name + " 妖精倉庫物品移出:" + item.getNumberedName(item.getCount(), false) + " OBJID:" + item.getId());
            list.remove(item);
            Connection con = null;
            PreparedStatement pstm = null;
            try {
