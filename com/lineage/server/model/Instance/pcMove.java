...
import static com.lineage.server.model.Instance.L1PcInstance.MOVE_TYPE;
...
public class pcMove extends NpcMoveExecutor {
    ...
    public void setDirectionMove(final int dir) {
        if (dir >= 0) {
            ...
            /* 精準目標特效跟隨 */
            _pc.setTrueTargetMoveSpeed();
            _pc.setSkinMoveSpeed();
            // 設置娃娃移動
            _pc.setNpcSpeed();
            // 寵物競速圈數檢查
            L1PolyRace.getInstance().checkLapFinish(_pc);
            if (result > 7) {
                L1PcUnlock.Pc_Unlock(_pc);
                result = 0;
            }
            result++;
            _pc.setSleepTimeAI(_pc.calcSleepTime(MOVE_TYPE));
        }
    }
    ...
    private int _check_targe_re8(int x, int y, double d) {
        ...
        //        _log.info("pcMove._check_targe_re8 再次取回方向無法通行，嘗試下一步動作。");
        return -1;
    }
        ...
        try {
            ...
            if (_aie <= _hce) {
                if (d <= L1NpcInstance.DISTANCE) {
                    ...
                    _cxy = maze.cxy();
                    _list = (Iterator<int[]>) maze.findBestPath().iterator();
                    if (!_list.hasNext()) {
                        _aie++;
                        _list = null;
                        _cxy = null;
    ...
    private boolean _exsistCharacterBetweenTarget(final int dir) {
        try {
            ...
            for (final Iterator<L1Object> iter = objects.iterator(); iter.hasNext(); ) {
                ...
                if ((object.getX() == targetX) && (object.getY() == targetY) && (object.getMapId() == _pc.getMapId())) {
                    isCheck = true;
                }
                if (isCheck) {
