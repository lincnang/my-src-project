...
public class HAND_DARKNESS extends SkillMode {
    ...
    public int start(final L1PcInstance srcpc, final L1Character cha, final L1Magic magic, final int integer) throws Exception {
        if (cha.hasSkillEffect(HAND_DARKNESS) || cha.hasSkillEffect(Phantom_Blade) || cha.hasSkillEffect(DESPERADO)) {
        ...
        int duration = integer; // 持續秒數

        // 設置 DESPERADO 恐怖狀態（可被恐怖耐性抵抗）
        targetPc.setSkillEffect(DESPERADO, duration * 1000);
        targetPc.setSkillEffect(HAND_DARKNESS, duration * 1000);
        ...
        // 發送恐怖狀態封包
        targetPc.sendPackets(new com.lineage.server.serverpackets.S_Paralysis(com.lineage.server.serverpackets.S_Paralysis.TYPE_DESPERADO, true));
    ...
    public void stop(final L1Character cha) throws Exception {
        if (cha instanceof L1PcInstance) {
            ...
            pc.flushedLocation();
            // 移除恐怖狀態封包
            pc.sendPackets(new com.lineage.server.serverpackets.S_Paralysis(com.lineage.server.serverpackets.S_Paralysis.TYPE_DESPERADO, false));
