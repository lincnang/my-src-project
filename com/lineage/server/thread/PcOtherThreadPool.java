...
public class PcOtherThreadPool {//src032
    ...
    private PcOtherThreadPool() {
        ...
        int _pcSchedulerPoolSize = 1 + Config.MAX_ONLINE_USERS / 10;
    ...
    /**
     * 週期性執行任務，首次執行完成後，在指定時間後再次執行，依次類推。
     */
    public ScheduledFuture<?> scheduleWithFixedDelay(final TimerTask command, final long initialDelay, final long period) {
        try {
            return _scheduler.scheduleWithFixedDelay(command, initialDelay, period, TimeUnit.MILLISECONDS);
        } catch (final RejectedExecutionException e) {
            _log.error(e.getLocalizedMessage(), e);
            return null;
        }
