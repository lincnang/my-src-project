 ...
 * 自動喝藥 -> 第二組
 */
public class SkillSoundHp2 extends TimerTask {
    private static Logger _log = Logger.getLogger(SkillSoundHp2.class.getName());
    ...
    public SkillSoundHp2(final L1PcInstance pc) {
        _pc = pc;
    }
            ...
            synchronized (this) {
                ...
                if (_pc.isAutoHp2()) {
                    ...
                    for (int i = 0; i < ItemId.length; i++) {
                        if (ItemId[i] == _pc.getAutoItemId2()) {
                            min = MinHp[i];
                            max = MaxHp[i];
                            gfx = GfxId[i];
                            break;
                    ...
                    if (((_pc.getMaxHp() / 100) * _pc.getTextHp2()) < _pc.getCurrentHp()) {
                        return;
                    }
                    if (_pc.getInventory().checkItem(_pc.getAutoItemId2())) {
                        if (L1BuffUtil.stopPotion(_pc)) { // 無法使用藥水
                            ...
                            _pc.getInventory().consumeItem(_pc.getAutoItemId2(), 1);
