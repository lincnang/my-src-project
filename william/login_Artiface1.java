            ...
            switch (type) {
                case 1: // 處理武器強化
                    ...
                    if (((Integer) aTempData.get(12)).intValue() == 1
                            && pc.get_other().getLv_Artifact() == ((Integer) aTempData.get(0)).intValue()
                    ...
                    if (pc.get_other().getLv_Artifact() == ((Integer) aTempData.get(0)).intValue()
                            && ((Integer) aTempData.get(12)).intValue() == 1) {
                        pc.sendPackets(new S_ServerMessage((String) aTempData.get(6)));
                    }

                    // 如果條件滿足，進行強化操作
                    if (pc.get_other().getLv_Artifact() == ((Integer) aTempData.get(0)).intValue()
                            && pc.get_other().getArtifact() >= ((Integer) aTempData.get(1)).intValue()
                            && ((Integer) aTempData.get(12)).intValue() == 1) {
                ...
                case 2: // 處理防具強化
                    ...
                    if (((Integer) aTempData.get(12)).intValue() == 0
                            && pc.get_other().getLv_Redmg_Artifact() == ((Integer) aTempData.get(2)).intValue()
                            && ((Integer) aTempData.get(10)).intValue() > 0) {
                        pc.sendPackets(new S_SystemMessage((String) aTempData.get(11)));
                        return;
                    }
                    ...
                    // 檢查當前等級是否符合強化條件，並發送提示消息
                    if (pc.get_other().getLv_Redmg_Artifact() == ((Integer) aTempData.get(2)).intValue()
                    ...
                    // 如果條件滿足，進行強化操作
                    if (pc.get_other().getLv_Redmg_Artifact() == ((Integer) aTempData.get(2)).intValue()
                            && pc.get_other().getArtifact1() >= ((Integer) aTempData.get(3)).intValue()
                            && ((Integer) aTempData.get(12)).intValue() == 0) {
        ...
        try {
            ...
            if (rs != null) {
                ...
                while (rs.next()) {
                    ...
                    aReturn.add(0, new Integer(rs.getInt("Lv_Artifact_weapon"))); // 武器等級
                    aReturn.add(1, new Integer(rs.getInt("Artifact_weapon"))); // 武器所需經驗
                    aReturn.add(2, new Integer(rs.getInt("Lv_Artifact_armor"))); // 防具等級
                    aReturn.add(3, new Integer(rs.getInt("Artifact_armor"))); // 防具所需經驗
                    aReturn.add(4, new Integer(rs.getInt("Lv_Artifact_other"))); // 其他等級（可能用於擴展）
                    aReturn.add(5, new Integer(rs.getInt("Artifact_other"))); // 其他所需經驗
                    aReturn.add(6, rs.getString("message")); // 提示消息
                    aReturn.add(7, rs.getString("message1")); // 成功消息
                    aReturn.add(8, new Integer(rs.getInt("check_world_mesage"))); // 是否檢查全服消息
                    aReturn.add(9, rs.getString("world_message")); // 全服廣播消息
                    aReturn.add(10, new Integer(rs.getInt("lv_stop"))); // 停止強化的等級
                    aReturn.add(11, rs.getString("stop_message")); // 停止強化的消息
                    aReturn.add(12, new Integer(rs.getInt("type"))); // 類型（1：武器，0：防具）
